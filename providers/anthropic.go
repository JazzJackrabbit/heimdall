...
func (a Anthropic) doRequest(
	...
	case models.AnthropicClaude4OpusAlias:
		...
		messages = append(messages, msgs...)
	case models.AnthropicClaude45HaikuAlias:
		msgs, err := prepareClaude45Haiku(
			req.Model,
			req.UserMessage,
		)
		if err != nil {
	...
	case models.AnthropicClaude45OpusAlias:
		...
		messages = append(messages, msgs...)
	case models.AnthropicClaude45SonnetAlias:
		msgs, err := prepareClaude45Sonnet(
			req.Model,
			req.UserMessage,
		)
		if err != nil {
...
func prepareClaude45Haiku(
	requestedModel models.Model,
	userMsg string,
) ([]anthropicMsg, error) {
	model, ok := requestedModel.(models.Claude45Haiku)
	if !ok {
		return nil, errors.New(
...
func prepareClaude45Sonnet(
	requestedModel models.Model,
	userMsg string,
) ([]anthropicMsg, error) {
	model, ok := requestedModel.(models.Claude45Sonnet)
	if !ok {
		return nil, errors.New(
